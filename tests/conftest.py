...
def assert_xml(tmp_path):
    def _assert_xml(feed, xpath, expected_text=None, expected_attrib=None):
        ...
        namespaces = { "itunes": "http://www.itunes.com/dtds/podcast-1.0.dtd", "podcast": "https://podcastindex.org/namespace/1.0" }
        ...
        if expected_text is not None:
            assert element.text == expected_text, f"Expected {element.tag} to contain {expected_text}, got {element.text}."

        if expected_attrib is not None:
            for key, value in expected_attrib.items():
                assert element.get(key) == value, f"Expected {key} to contain {value}, got {element.get(key)}."
